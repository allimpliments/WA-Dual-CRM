const Social = {
  ...
  async render() {
    ...
    let fbConfig = {}, igConfig = {}, scheduledPosts = [];
    try {
    ...
    this.loadTabContent(fbConfig, igConfig, scheduledPosts);
  },
  ...
  loadTabContent(fbConfig, igConfig, scheduledPosts) {
    const container = document.getElementById('socialTabContent');
    if (!container) return;

    if (this.currentTab === 'create-post') {
      container.innerHTML = `
  ...
  async postNow() {
    ...
    if (platform === 'facebook') {
      ...
      } catch (err) { document.getElementById('postResult').innerHTML = `<span class="text-danger">❌ ${err.message}</span>`; }
