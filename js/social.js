const Social = {
  ...
  fbPages: [],
  igAccounts: [],
  postType: 'post', // 'post', 'reel', 'story', 'carousel', 'catalogue'
            ...
            <div class="d-flex gap-3">
              ...
              <div class="flex-grow-1">
                <div class="d-flex justify-content-between">
                  <div><span class="badge bg-${p.platform==='facebook'?'primary':'danger'}">${p.platform}</span> <span class="badge bg-secondary">${p.pageName||p.accountName||''}</span> <span class="badge bg-info">${p.postType||'post'}</span></div>
        ...
        <div style="background:#f0f2f5;border-radius:16px;padding:32px;max-height:92vh;overflow-y:auto;width:96%;max-width:1300px;" onclick="event.stopPropagation()">
          ...
          <div style="display:grid;grid-template-columns:1fr 400px;gap:24px;">
            <div>
              ...
              <!-- Post Type Selector -->
              <div class="meta-card">
                <div class="fw-bold mb-3">Post Type</div>
                <select id="postType" class="form-select" onchange="Social.postType = this.value;" style="border-radius:8px;">
  ...
  async savePost(status) {
    ...
    const postType = document.getElementById('postType')?.value || this.postType;
    ...
    for(const platform of ['facebook','instagram']) {
      ...
      if(status === 'published') {
        if(platform === 'facebook') {
          ...
          if(this.uploadedFiles.length === 0) { alert('Instagram requires at least one image or video!'); return; }
          const igUserId = cfg.accountId;

          // Determine media_type based on postType
          let mediaType = '';
          if (postType === 'reel' || postType === 'story') {
            mediaType = postType.toUpperCase(); // "REEL" -> "REELS"? Instagram expects "REELS" for reels, "STORIES" for stories
            if (mediaType === 'REEL') mediaType = 'REELS';
            if (mediaType === 'STORY') mediaType = 'STORIES';
          } else if (postType === 'catalogue') {
            // Catalogue posting requires product catalog, we'll skip for now
            alert('Catalogue posting will be available soon.');
            return;
          }
          // For post and carousel, media_type is not needed (or can be omitted)
          try {
            for(const url of this.uploadedFiles.slice(0, 10)) {
              ...
              if (isVideo || postType === 'reel' || postType === 'story') {
              ...
              if(createData.id) {
                // Publish for all except stories? Stories might auto-publish or need different endpoint. We'll attempt media_publish for all.
  ...
  async publishDraft(id) {
    ...
    if(post.platform==='instagram'){
      ...
      for(const url of (post.media||[]).slice(0,10)){
        ...
        if(isVideo || postType === 'reel' || postType === 'story') {
          params.append('media_type', mediaType || (isVideo ? 'VIDEO' : 'IMAGE'));
          if(isVideo) params.append('video_url', url);
