const Social = {
  ...
  async render() {
    ...
    let html = `
      <style>
        ...
        .social-sidebar { width: 220px; background: #fff; border-right: 1px solid #e0e0e0; overflow-y: auto; flex-shrink: 0; }
        .social-sidebar .platform-item { padding: 12px 16px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: 500; font-size: 13px; border-bottom: 1px solid #f0f0f0; transition: 0.15s; }
        ...
        .social-sidebar .status-dot { width: 8px; height: 8px; border-radius: 50%; }
        .social-sidebar .status-dot.saved { background: #31a24c; }
        .social-main { flex: 1; display: flex; flex-direction: column; background: #f0f2f5; }
        .social-toolbar { padding: 8px 14px; background: #fff; border-bottom: 1px solid #e0e0e0; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; gap: 10px; }
        .social-toolbar .nav-btns { display: flex; gap: 4px; }
        .social-toolbar .nav-btns button { width: 30px; height: 30px; border: 1px solid #dadde1; background: #fff; border-radius: 6px; cursor: pointer; font-size: 14px; }
        .social-toolbar .url-bar { flex: 1; padding: 7px 12px; border: 1px solid #dadde1; border-radius: 8px; font-size: 13px; background: #fafbfc; }
        .social-iframe-wrap { flex: 1; position: relative; background: #fff; }
        .social-iframe-wrap iframe { width: 100%; height: 100%; border: none; }
        .social-iframe-wrap .iframe-placeholder { position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: #fafbfc; flex-direction: column; }
        .save-row { display: flex; gap: 6px; align-items: center; }
        .save-row input { padding: 6px 8px; border: 1px solid #dadde1; border-radius: 6px; font-size: 12px; width: 130px; }
      </style>
      <div class="social-container">
        <div class="social-sidebar">
          <div style="padding:14px 16px;font-weight:700;font-size:12px;color:#65676b;text-transform:uppercase;">Platforms</div>
          ${platforms.map(p => `
            <div class="platform-item ${this.currentTab === p.id ? 'active' : ''}" onclick="Social.switchTab('${p.id}')">
              <i class="fab ${p.icon}" style="color:${p.color};width:18px;text-align:center;"></i> ${p.name}
              <span class="status-dot ${this.savedAccounts[p.id] ? 'saved' : ''}" style="margin-left:auto;background:${this.savedAccounts[p.id] ? '#31a24c' : '#dadde1'};"></span>
            </div>
          `).join('')}
        </div>
        <div class="social-main">
          <div class="social-toolbar">
            <div class="nav-btns">
              <button onclick="Social.navigateIframe('back')" title="Back">←</button>
              <button onclick="Social.navigateIframe('forward')" title="Forward">→</button>
              <button onclick="Social.navigateIframe('reload')" title="Reload">↻</button>
              <button onclick="Social.navigateIframe('home')" title="Home">⌂</button>
            </div>
          ...
          <div class="social-iframe-wrap" id="iframeContainer">
            <div class="iframe-placeholder" id="iframePlaceholder" style="display:${this.savedAccounts[this.currentTab] ? 'none' : 'flex'};">
              ...
              <p class="text-muted">Click "Open" to load in new tab (framing blocked by this site).</p>
              <button class="btn btn-primary mt-2" onclick="Social.openInNewTab('${this.currentTab}')">🔗 Open ${this.getPlatformName(this.currentTab)}</button>
            </div>
            <iframe id="socialIframe" style="display:none;" src="about:blank" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox allow-top-navigation"></iframe>
          </div>
        </div>
      </div>
    `;
    contentArea.innerHTML = html;
    document.getElementById('iframeUrl').value = this.getPlatformUrl(this.currentTab);
  },
  ...
  navigateIframe(action) {
    // Since iframes don't work, this is a placeholder
  },
